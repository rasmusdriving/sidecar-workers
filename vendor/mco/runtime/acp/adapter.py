...
class AcpAdapter:
    ...
    def _make_client(self, task, command, stderr_path):
        return AcpClient(command=command, cwd=task.repo_root, stderr_path=stderr_path)
    ...
    def run(self, input_task: TaskInput) -> TaskRunRef:
        ...
        client = self._make_client(input_task, acp_cmd, stderr_path)
        ...
        except (JsonRpcError, TransportClosed, TimeoutError, OSError, ValueError) as exc:
            client.close()
        ...
        def _run_prompt() -> None:
            try:
                ...
                handle.success = True
            except (JsonRpcError, TransportClosed, TimeoutError, RuntimeError) as exc:

...
def effective_provider_risk(
    ...
    if provider == "devin" and permissions.get("mode") in ("ask", "accept-edits", "smart"):
        return {"level": "read_only" if permissions["mode"] == "ask" else "workspace_write",
                "reason": "Devin ACP mode is verified; remaining permissions go to the coordinating agent"}
